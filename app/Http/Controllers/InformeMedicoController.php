...
class InformeMedicoController extends Controller
{
    public function verRegistros(Request $request)
    {
        ...
        if(!empty($request->informe_medico))
            $filtros[] = array('informe_medico))','like', '%'.$request->informe_medico.'%');
        if(!empty($request->fecha_informe_medico))
            $filtros[] = array('fecha_informe_medico', $request->fecha_informe_medico);
        ...
        $registros = InformeMedico::where($filtros)->get();
