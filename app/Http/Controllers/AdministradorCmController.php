...
class AdministradorCmController extends Controller
{
    ...
    {
        return redirect()->route('adm_cm.profesionales');
    }
    ...
    public function examenes(){
        ...
        return view('app.adm_cm.procedimientos_funcionales',[
            'institucion' => $institucion,
            'tipo_institucion' => $tipo_institucion,
    ...
    public function vacunatorio_agenda()
    {
        $profesional = Profesional::where('id_usuario', Auth::id())->first();

        if (!$profesional) {
            return redirect()->route('adm_cm.personal')
                ->with('error', 'Debe asociar este usuario a un profesional del vacunatorio para abrir su agenda.');
        }

        $lugarAtencion = $profesional->LugaresAtencion()->first();

        if (!$lugarAtencion) {
            return redirect()->route('adm_cm.personal')
                ->with('error', 'El profesional no tiene un lugar de atención asociado.');
        }

        return redirect()->route('profesional.mi_agenda', [
            'lugares_atencion' => $lugarAtencion->id,
        ]);
    }
