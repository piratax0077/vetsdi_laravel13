...
class CertificadoReposoController extends Controller
{
    public function verRegistros(Request $request)
    {
        ...
        if(!empty($request->fecha_inicio))
            $filtros[] = array('fecha_inicio', $request->fecha_inicio);
        if(!empty($request->fecha_termino))
            $filtros[] = array('fecha_termino', $request->fecha_termino);
        if(!empty($request->hipotesis))
            $filtros[] = array('hipotesis', 'like', $request->hipotesis.'%');
        if(!empty($request->comentarios))
            $filtros[] = array('comentarios','like', $request->comentarios.'%');
        ...
        $registros = CertificadoReposo::where($filtros)->get();
