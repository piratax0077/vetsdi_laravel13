@php
    $nombreEspecieMaxilar = (string) (
        data_get($mascota ?? null, 'especieMascota.nombre')
        ?? data_get($mascota ?? null, 'especie_nombre')
        ?? data_get($mascota ?? null, 'especie')
        ?? ''
    );
    $especieMaxilarNormalizada = strtolower(\Illuminate\Support\Str::ascii($nombreEspecieMaxilar));
    $esMaxilarFelino = str_contains($especieMaxilarNormalizada, 'felin')
        || str_contains($especieMaxilarNormalizada, 'gato');
    $tipoMaxilarVeterinario = $esMaxilarFelino ? 'Felino' : 'Canino';
    $imagenMaxilarSuperior = $esMaxilarFelino
        ? 'img_dental/maxilar_superior_felino.png'
        : 'img_dental/maxilar_superior_canino.png';
@endphp

@once
    <style>
        .maxilar-veterinario-superior {
            display: block;
            width: auto;
            max-width: 100%;
            max-height: 210px;
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-info">
                <h5 class="modal-title text-white">Maxilar superior {{ strtolower($tipoMaxilarVeterinario) }}</h5>
                ...
                <form id="form_diagnostico_tratamiento">
                    <div class="row">
                        ...
                        <div class="col-md-4 text-center">
                            <img src="{{ asset($imagenMaxilarSuperior) }}"
                                class="img-fluid maxilar-veterinario-superior"
                                alt="Maxilar superior veterinario {{ strtolower($tipoMaxilarVeterinario) }}">
                        </div>
                    </div>
                    <hr>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-info" id="btn_guardar_maxilar_superior"
                    onclick="guardarTodoMaxilarSuperior()">
                    ...
                    <div class="tab-pane fade active show" id="diag_max_sup" role="tabpanel" aria-labelledby="diag_max_sup-tab">
                        ...
                        <form class="mt-5">
                            <div class="row">
                                ...
                                <div class="col-md-4 text-center">
                                    <img src="{{ asset($imagenMaxilarSuperior) }}" class="maxilar-veterinario-superior"
                                        alt="Maxilar superior veterinario {{ strtolower($tipoMaxilarVeterinario) }}">
                                </div>
                            </div>
                            <hr>
...
<script>
    ...
    function guardarTodoMaxilarSuperior() {
        const filas = $('#tbody_diagnosticos_tratamientos tr').length;

        if (filas === 0) {
            swal({
                title: 'Sin registros',
                text: 'Primero agregue al menos un diagnóstico y tratamiento.',
                icon: 'warning'
            });
            return;
        }

        $('#tratamiento_maxilar_superior').modal('hide');
        swal({
            title: 'Maxilar superior guardado',
            text: 'Los diagnósticos y tratamientos quedaron registrados en la ficha odontológica.',
            icon: 'success',
            timer: 1800,
            buttons: false
        });
