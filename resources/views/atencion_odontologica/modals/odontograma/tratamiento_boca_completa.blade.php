                ...
                <form id="form_diagnostico_tratamiento_inferior">
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <div class="form-group">
                                <label class="floating-label-activo-sm">Prestación del profesional (UCOV)</label>
                                <select class="form-control form-control-sm" id="tarifa_ucov_boca_completa">
                                    <option value="">Cargando aranceles del profesional...</option>
                                </select>
                                <small id="detalle_ucov_boca_completa" class="form-text text-info"></small>
                            </div>
                            <div class="form-group">
                                <label class="floating-label-activo-sm">Tratamiento seleccionado o manual</label>
                                ...
                                <small class="form-text text-muted">Al seleccionar una prestación UCOV, su valor se incorporará al presupuesto dental del profesional.</small>
                            </div>
                            <div class="form-group">
...
<script>
    let catalogoUcovBocaCompleta = [];

    function cargarCatalogoUcovBocaCompleta() {
        const selector = $('#tarifa_ucov_boca_completa');
        if (!selector.length) return;

        const lugarAtencion = $('#id_lugar_atencion').val() || '';
        selector.prop('disabled', true).html('<option value="">Cargando aranceles del profesional...</option>');

        $.ajax({
            type: 'GET',
            url: "{{ route('veterinaria.odontologia.tarifario.index') }}",
            data: { id_lugar_atencion: lugarAtencion },
            dataType: 'json',
            success: function (response) {
                catalogoUcovBocaCompleta = response.prestaciones || [];
                let html = '<option value="">Ingreso manual (sin arancel UCOV)</option>';
                let categoriaActual = null;

                catalogoUcovBocaCompleta.forEach(function (item) {
                    if (item.categoria !== categoriaActual) {
                        if (categoriaActual !== null) html += '</optgroup>';
                        categoriaActual = item.categoria || 'Prestaciones odontológicas';
                        html += '<optgroup label="' + $('<div>').text(categoriaActual).html() + '">';
                    }
                    html += '<option value="' + item.id + '">' +
                        $('<div>').text(item.nombre).html() + ' · ' +
                        Number(item.uco || 0).toLocaleString('es-CL') + ' UCOV · $' +
                        Math.round(item.valor_total || 0).toLocaleString('es-CL') + '</option>';
                });
                if (categoriaActual !== null) html += '</optgroup>';

                selector.html(html).prop('disabled', false);
                if (!catalogoUcovBocaCompleta.length) {
                    $('#detalle_ucov_boca_completa').text('El profesional aún no tiene aranceles UCOV configurados para este lugar de atención.');
                }
            },
            error: function () {
                selector.html('<option value="">Ingreso manual (catálogo no disponible)</option>').prop('disabled', false);
                $('#detalle_ucov_boca_completa').text('No fue posible cargar el tarifario UCOV. Puede registrar el tratamiento manualmente.');
            }
        });
    }

    $(document).on('change', '#tarifa_ucov_boca_completa', function () {
        const tarifa = catalogoUcovBocaCompleta.find(item => String(item.id) === String(this.value));
        if (!tarifa) {
            $('#detalle_ucov_boca_completa').text('');
            return;
        }
        $('#diag_seleccionado_boca_compl_gral_autocomplete').val(tarifa.nombre);
        $('#detalle_ucov_boca_completa').html(
            '<strong>' + Number(tarifa.uco || 0).toLocaleString('es-CL') + ' UCOV</strong> × $' +
            Math.round(tarifa.valor_uco || 0).toLocaleString('es-CL') +
            ' = <strong>$' + Math.round(tarifa.valor_total || 0).toLocaleString('es-CL') + '</strong>'
        );
    });

    $('#tratamiento_boca_completa').on('show.bs.modal', cargarCatalogoUcovBocaCompleta);
    ...
    const tarifaUcovId = parseInt($('#tarifa_ucov_boca_completa').val() || 0, 10) || null;
    const tarifaUcov = tarifaUcovId
        ? catalogoUcovBocaCompleta.find(item => Number(item.id) === tarifaUcovId)
        : null;
    ...
    $.ajax({
        ...
        success: function (response) {
            if (tarifaUcovId) {
                const mascotaId = parseInt(new URLSearchParams(window.location.search).get('id_mascota') || 0, 10);
                $.ajax({
                    type: 'POST',
                    url: "{{ route('veterinaria.odontologia.presupuesto.guardar') }}",
                    data: {
                        mascota_id: mascotaId,
                        paciente_id: id_paciente,
                        ficha_atencion_id: id_ficha_atencion,
                        lugar_atencion_id: id_lugar_atencion,
                        pieza: 0,
                        tarifa_uco_id: tarifaUcovId,
                        observaciones: comentarios,
                        _token: CSRF_TOKEN
                    },
                    success: function (presupuestoResponse) {
                        const resumen = presupuestoResponse.presupuesto || {};
                        swal({
                            title: 'Tratamiento presupuestado',
                            text: (tarifaUcov ? tarifaUcov.nombre : tratamiento) + '\nTotal presupuesto: $' +
                                Math.round(resumen.total || 0).toLocaleString('es-CL') + '\nSaldo: $' +
                                Math.round(resumen.saldo || 0).toLocaleString('es-CL'),
                            icon: 'success'
                        });
                    },
                    error: function (xhr) {
                        const mensaje = xhr.responseJSON && (xhr.responseJSON.message || xhr.responseJSON.msj)
                            ? (xhr.responseJSON.message || xhr.responseJSON.msj)
                            : 'El tratamiento clínico se guardó, pero no pudo incorporarse al presupuesto UCOV.';
                        swal({ title: 'Presupuesto pendiente', text: mensaje, icon: 'warning' });
                    }
                });
            } else {
