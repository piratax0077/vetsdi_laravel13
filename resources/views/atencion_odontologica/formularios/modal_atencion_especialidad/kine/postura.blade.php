<div id="postura" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="postura" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-info">
                <h5 class="modal-title mt-1" id="modal_ofa">Examen Motor Postura y Marcha</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                        <ul class="nav nav-tabs-aten nav-fill mb-3" id="postura" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link-aten text-reset active" id="pos-seg-sup-tab" data-toggle="tab" href="#pos-seg-sup" role="tab" aria-controls="pos-seg-sup" aria-selected="true">Seg. superior</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link-aten text-reset" id="pos-seg-med-tab" data-toggle="tab" href="#pos-seg-med" role="tab" aria-controls="pos-seg-med" aria-selected="false">Seg. medio</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link-aten text-reset" id="pos-seg-inf-tab" data-toggle="tab" href="#pos-seg-inf" role="tab" aria-controls="pos-seg-inf" aria-selected="false">Seg. inferior</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link-aten text-reset" id="pos-pat-post-tab" data-toggle="tab" href="#pos-pat-post" role="tab" aria-controls="pos-pat-post" aria-selected="false">Patrón postural</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link-aten text-reset" id="post-pat-march-tab" data-toggle="tab" href="#post-pat-march" role="tab" aria-controls="post-pat-march" aria-selected="false">Patrón de marcha</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                        <div class="tab-content" id="postura">
                            <!--SEG.SUPERIOR-->
                            <div class="tab-pane fade show active" id="pos-seg-sup" role="tabpanel" aria-labelledby="pos-seg-sup-tab">
                                <div class="form-row">
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <h6 class="text-c-blue mb-3"> SEG.SUPERIOR</h6>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Anterior</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Posterior</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Lateral</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <div class="form-group">
                                           <label class="floating-label">Conclusiones</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--SEG.MEDIOS-->
                            <div class="tab-pane fade show" id="pos-seg-med" role="tabpanel" aria-labelledby="pos-seg-med-tab">
                                <div class="form-row">
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <h6 class="text-c-blue mb-3"> SEG.MEDIOS</h6>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Anterior</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Posterior</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Lateral</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <div class="form-group">
                                           <label class="floating-label">Conclusiones</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--SEG.INFERIOR-->
                            <div class="tab-pane fade show" id="pos-seg-inf" role="tabpanel" aria-labelledby="pos-seg-inf-tab">
                                <div class="form-row">
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <h6 class="text-c-blue mb-3"> SEG.INFERIOR</h6>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Anterior</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Posterior</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
                                        <div class="form-group">
                                           <label class="floating-label">Lateral</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <div class="form-group">
                                           <label class="floating-label">Conclusiones</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=6" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--PATRÓN POSTURAL-->
                            <div class="tab-pane fade show" id="pos-pat-post" role="tabpanel" aria-labelledby="pos-pat-post-tab">
                                <div class="form-row">
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <h6 class="text-c-blue mb-3">PATRÓN POSTURAL</h6>
                                    </div>
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <div class="form-group">
                                           <label class="floating-label">Evaluación</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=4" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!--PATRÓN POSTURAL-->
                            <div class="tab-pane fade show" id="post-pat-march" role="tabpanel" aria-labelledby="post-pat-march-tab">
                                <div class="form-row">
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <h6 class="text-c-blue mb-3">PATRÓN DE MARCHA</h6>
                                    </div>
                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                        <div class="form-group">
                                           <label class="floating-label">Evaluación</label>
                                           <textarea class="form-control form-control-sm" rows="1" onfocus="this.rows=4" onblur="this.rows=1;" name="flex-derecha" id="flex-derecha"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal"><i class="feather icon-x"></i> Cerrar</button>
                <button type="button" class="btn btn-info btn-sm"><i class="feather icon-save"></i> Guardar</button>
            </div>
        </div>
    </div>
</div>