...
<div class="pcoded-main-container"><div class="pcoded-content"><div class="ref-shell">
    <div class="ref-hero mb-4"><div class="row align-items-center"><div class="col-md-8">
        ...
        <p class="mb-0">Invita a tutores, veterinarios, centros, comercios de alimentación o farmacias veterinarias. Comparte por correo o WhatsApp y sigue tus puntos.</p>
    ...
    <div class="row">
        <div class="col-lg-7 mb-4"><div class="card ref-panel h-100"><div class="card-header"><h5 class="text-white mb-0">Nueva invitación</h5></div><div class="card-body">
            <form method="POST" action="{{ route('paciente.referidos.store') }}">@csrf
                <div class="form-row"><div class="form-group col-md-6"><label>Quiero invitar</label><select class="form-control" name="tipo" id="tipo_referido" required><option value="tutor">A otro tutor de mascotas</option><option value="profesional">A un veterinario</option><option value="centro">A un centro veterinario</option><option value="alimentacion">A un comercio de alimentación</option><option value="farmacia">A una farmacia veterinaria</option></select></div>
                <div class="form-group col-md-6"><label>Nombre del contacto</label><input class="form-control" name="nombre_invitado" value="{{ old('nombre_invitado') }}" required></div></div>
                ...
                <button class="btn btn-info px-4"><i class="feather icon-send mr-1"></i>Registrar y enviar correo</button>
            </form>
        </div></div></div>
        <div class="col-lg-5 mb-4"><div class="card ref-panel h-100"><div class="card-header"><h5 class="text-white mb-0">Esquema de incentivo</h5></div><div class="card-body">
            ...
            <div class="step"><b>2</b><div><strong>Primera activación</strong><br><small>200 puntos adicionales cuando empieza a utilizar VET SDI.</small></div></div>
            <div class="step"><b>3</b><div><strong>Comercio o centro activo</strong><br><small>500 puntos si el centro, comercio de alimentación o farmacia completa su primera operación.</small></div></div>
            <div class="alert alert-light border mb-0"><strong>Conversión sugerida:</strong> 100 puntos = $1.000 de crédito, aplicable hasta al 50% del plan mensual.</div>
        </div></div></div>
    </div>
    ...
    <div class="card ref-panel"><div class="card-header"><h5 class="text-white mb-0">Seguimiento de invitaciones</h5></div><div class="card-body table-responsive">
        <table class="table table-hover"><thead><tr><th>Invitado</th><th>Tipo</th><th>Código</th><th>Fecha</th><th>Estado</th><th>Puntos</th><th>Acciones</th></tr></thead><tbody>
        @forelse($referidos as $referido)
            @php($wa = preg_replace('/\D+/', '', (string) $referido->telefono_invitado))
            @php($mensaje = 'Hola '.$referido->nombre_invitado.', te invito a VET SDI: '.route('referidos.aceptar', $referido->token).' Código: '.$referido->codigo)
            <tr><td><strong>{{ $referido->nombre_invitado }}</strong><br><small>{{ $referido->email_invitado }}</small>@if($referido->telefono_invitado)<br><small>{{ $referido->telefono_invitado }}</small>@endif</td>
            <td>{{ ['centro'=>'Centro veterinario','tutor'=>'Tutor','profesional'=>'Veterinario','alimentacion'=>'Alimentación','farmacia'=>'Farmacia veterinaria'][$referido->tipo] ?? ucfirst($referido->tipo) }}@if($referido->nombre_centro)<br><small>{{ $referido->nombre_centro }}</small>@endif</td><td>{{ $referido->codigo }}</td><td>{{ optional($referido->fecha_envio)->format('d-m-Y') }}</td><td><span class="state state-{{ $referido->estado }}">{{ ucfirst($referido->estado) }}</span></td><td>{{ $referido->puntos_otorgados }} / {{ $referido->puntos_propuestos }}</td>
            <td>@if($wa)<a class="btn btn-success btn-sm mb-1" href="https://wa.me/{{ $wa }}?text={{ urlencode($mensaje) }}" target="_blank" rel="noopener"><i class="feather icon-message-circle"></i> WhatsApp</a>@endif @if(!in_array($referido->estado,['activada','bonificada','cancelada']))<form method="POST" class="d-inline" action="{{ route('paciente.referidos.cancelar',$referido) }}">@csrf @method('PATCH')<button class="btn btn-outline-danger btn-sm" onclick="return confirm('¿Cancelar esta invitación?')">Cancelar</button></form>@endif</td></tr>
        @empty<tr><td colspan="7" class="text-center text-muted py-4">Aún no has enviado invitaciones.</td></tr>@endforelse
        </tbody></table>
    </div></div>
</div></div></div>
<script>document.addEventListener('DOMContentLoaded',function(){var tipo=document.getElementById('tipo_referido'),grupo=document.getElementById('grupo_nombre_centro'),input=grupo.querySelector('input'),etiqueta=grupo.querySelector('label');function actualizar(){var organizacion=['centro','alimentacion','farmacia'].indexOf(tipo.value)!==-1;grupo.style.display=organizacion?'block':'none';input.required=organizacion;etiqueta.textContent=tipo.value==='farmacia'?'Nombre de la farmacia':(tipo.value==='alimentacion'?'Nombre del comercio':'Nombre del centro');}tipo.addEventListener('change',actualizar);actualizar();});</script>
@endsection
