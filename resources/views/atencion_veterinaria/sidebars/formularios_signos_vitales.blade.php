                    ...
                    <div class="card-body-sidebar">
                        <div class="form-row pt-3 px-2">
                            ...
                            <div class="form-group col-sm-12 col-md-6 col-lg-6 col-xl-6">
                                ...
                                <input type="text" class="form-control form-control-sm" name="presion_pie_sidebar" id="presion_pie_sidebar" value="" readonly>
                            </div>
                            <div class="form-group col-sm-12 col-md-6 col-lg-6 col-xl-6">
...
<script>
    function calcularPamVeterinariaSidebar() {
        let pas = Number($('#presion_bi_sidebar').val());
        let pad = Number($('#presion_bd_sidebar').val());

        if (!Number.isFinite(pas) || !Number.isFinite(pad) || pas <= 0 || pad <= 0 || pas < pad) {
            $('#presion_pie_sidebar').val('');
            return;
        }

        let pam = pad + ((pas - pad) / 3);
        $('#presion_pie_sidebar').val(Number.isInteger(pam) ? pam : pam.toFixed(1));
    }

    $(document).ready(function(){
        $('#imc_sidebar').val(@json(isset($fichaAtencion) ? $fichaAtencion->imc : ''));
        calcularPamVeterinariaSidebar();
    });

    $(document).on('input', '#presion_bi_sidebar, #presion_bd_sidebar', calcularPamVeterinariaSidebar);
