...
@section('modals-med-exa')
    @include('general.secciones_ficha.receta_examen.modal_recetario_sdi_gine')
...
@section('page-script-med-exa')
    ...
    <script>
        ...
        function registro_examen_ficha() {
            ...
            $('#tabla_examen_cirugia_d tr').each(function(i, n) {
                if (i > 0) {
                    ...
                    rol["fecha_hora"] = $.trim($(data[1]).text().split("\n").join(""));
                    rol["nombre_examen"] = $.trim($(data[2]).text().split("\n").join(""));
            ...
            let id_ficha_gineco_obstetrica = $('#id_fc').val();
        ...
        function ver_pdf_orden_examenes(id_ficha_atencion)
        {
            ...
            let url = "{{ route('pdf.orden_examenes.gine') }}";
            @if(!empty(session('lic_token')) && session('lic_estado') == 1)
