    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-info">
                <h5 class="modal-title text-white mt-1">{{ isset($mascota) && $mascota ? 'Receta Veterinaria' : 'Recetario SDI' }}</h5>
                    ...
                    <div class="alert alert-info py-2 mb-3">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <small>{{ optional($mascota->especieMascota)->nombre ?? $mascota->especie ?? 'Especie sin registro' }}@if(!empty($mascota->chip)) · Chip: {{ $mascota->chip }}@endif</small>
                            </div>
                            <div class="col-md-6">
                                <strong>Tutor:</strong> {{ trim(($paciente->nombres ?? '').' '.($paciente->apellido_uno ?? '').' '.($paciente->apellido_dos ?? '')) }}<br>
                                <small>RUT: {{ $paciente->rut ?? 'Sin registro' }}</small>
                            </div>
                        </div>
                    </div>
                @endif
                                ...
                                <div class="col-md-12">
                                    <div class="col-sm-12 mt-3 mb-2">
                                        <div class="custom-control custom-switch">
                                            ...
                                            <label class="custom-control-label text-primary" for="ranking_recetas_switch"><strong><u>Ranking de recetas controladas {{ isset($mascota) && $mascota ? 'de la mascota' : 'del paciente' }}</u></strong></label>
                                        </div>
                                    </div>
                                    <div class="row" id="ranking_recetas" style="display:none">
                                        <div class="col-sm-12">
                                            <h6 class="text-c-blue mb-3">Indicaciones con receta controlada del año {{ date('Y') }}</h6>
                                        </div>
                                        <div class="col-sm-12 col-md-6">
                                            <div class="form-group fill">
                                                <label class="floating-label">Tipo de control</label>
                                                <select class="form-control form-control-sm" id="tipo_control_ranking_recetas">
                                                    <option value="0">Seleccione una opción</option>
                                                    @foreach ($receta_control as $tipo_receta_control)
                                                        @if(in_array((int) $tipo_receta_control->tipo_control, [2, 3, 4], true))
                                        ...
                                        <div class="col-4 col-md-2">
                                            <div class="form-group">
                                                <label class="floating-label-activo-sm">Este veterinario</label>
                                                <input class="form-control form-control-sm text-center" type="text" id="ranking_receta_propia_cantidad" value="0" readonly>
                                            </div>
                                        </div>
                                        <div class="col-4 col-md-2">
                                            <div class="form-group">
                                                <label class="floating-label-activo-sm">Otros veterinarios</label>
                                                <input class="form-control form-control-sm text-center" type="text" id="ranking_receta_otros_cantidad" value="0" readonly>
                                            </div>
                                        </div>
                                        <div class="col-4 col-md-2">
                                            <div class="form-group">
                                                <label class="floating-label-activo-sm">Total {{ isset($mascota) && $mascota ? 'mascota' : 'paciente' }}</label>
                                                <input class="form-control form-control-sm text-center font-weight-bold" type="text" id="ranking_receta_total_cantidad" value="0" readonly>
                                            </div>
                                        </div>
                                        <div class="col-sm-12 mb-2">
                                            <small class="text-muted">Incluye indicaciones del veterinario actual y de otros veterinarios.</small>
                                        </div>
...
<script>
    var creatinina = 0;
        ...
        $("#nombre_medicamento_ficha_dental").autocomplete({
            ...
            select: function(event, ui) {
                ...
                if ([2, 3, 4].indexOf(parseInt(ui.item.control, 10)) !== -1) {
                    cargarRankingRecetasControladas(ui.item.control, true);
                }
    ...
    function buscar_cantidad_receta_propia()
    {
        ...
        var id_mascota = $('#id_mascota_fc').val() || '';
    ...
    function cargarRankingRecetasControladas(tipoControl, actualizarMensaje)
    {
        var control = parseInt(tipoControl || $('#tipo_control_ranking_recetas').val(), 10);
        var idPaciente = $('#id_paciente_fc').val();
        var idMascota = $('#id_mascota_fc').val() || '';
        var idProfesional = $('#id_profesional_fc').val();

        $('#ranking_receta_propia_cantidad').val(0);
        $('#ranking_receta_otros_cantidad').val(0);
        $('#ranking_receta_total_cantidad').val(0);

        // En veterinaria solamente los tipos 2, 3 y 4 corresponden a
        // prescripciones sujetas a control y deben integrar el ranking.
        if ([2, 3, 4].indexOf(control) === -1 || !idPaciente) {
        ...
        $.ajax({
            ...
            if (parseInt(data.estado, 10) !== 1) {
                return;
            }

            $('#ranking_receta_propia_cantidad').val(data.cantidad_propia || 0);
            $('#ranking_receta_otros_cantidad').val(data.cantidad_otros || 0);
            $('#ranking_receta_total_cantidad').val(data.cantidad_total || 0);

            if (actualizarMensaje) {
                var cantidad = parseInt(data.cantidad_total || 0, 10);
                if (cantidad > 0) {
                    $('#mensaje_med_control').html(
                        (idMascota ? 'Esta mascota registra ' : 'Este paciente registra ') + cantidad +
                        (cantidad === 1 ? ' receta controlada' : ' recetas controladas') +
                        ' durante el año {{ date('Y') }}.<br>Consulte el ranking de recetas controladas.'
                    );
                } else {
                    $('#mensaje_med_control').html('');
                }
            ...
            console.log('[ranking recetas controladas]', jqXHR.responseJSON || jqXHR.responseText);
            if (actualizarMensaje) {
                $('#mensaje_med_control').html('');
            }
        });
    ...
    $(document).on('change', '#ranking_recetas_switch', function() {
        var visible = $(this).is(':checked');
        $('#ranking_recetas').toggle(visible);
        if (visible) {
            var controlActual = parseInt($('#id_medicamento_tipo_control').val(), 10);
            if ([2, 3, 4].indexOf(controlActual) !== -1) {
                $('#tipo_control_ranking_recetas').val(controlActual);
            }
            cargarRankingRecetasControladas($('#tipo_control_ranking_recetas').val(), false);
        }
    });

    $(document).on('change', '#tipo_control_ranking_recetas', function() {
        cargarRankingRecetasControladas($(this).val(), false);
    });
