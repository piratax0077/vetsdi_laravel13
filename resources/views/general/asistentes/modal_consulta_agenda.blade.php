                ...
                <div class="tab-content" id="pills-tabContent-interconsulta">
                    ...
                    <div class="tab-pane fade show active" id="pills-recibir-bono" role="tabpanel"
                        ...
                        <div class="form-row">
                                ...
                                <div class="form-group fill">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control form-control-sm" name="bono_numero"
                                            id="bono_numero" autocomplete="off">
                                        <div class="input-group-append">
                                            <button type="button" class="btn btn-outline-info btn-sm"
                                                id="btn_escanear_qr_pago" onclick="iniciarLectorQrPagoAgenda()">
                                                <i class="fas fa-qrcode"></i> Leer QR
                                            </button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-sm-12">
                                <div id="panel_lector_qr_pago" class="card border-info mb-3" style="display:none;">
                                    <div class="card-body p-2 text-center">
                                        <video id="video_lector_qr_pago" class="w-100 rounded bg-dark"
                                            style="max-height:280px;" playsinline muted></video>
                                        <div id="estado_lector_qr_pago" class="small text-muted mt-2" aria-live="polite">
                                            Apunte la cámara al QR emitido en Bonos Veterinaria.
                                        </div>
                                        <button type="button" class="btn btn-secondary btn-sm mt-2"
                                            onclick="detenerLectorQrPagoAgenda()">
                                            <i class="fas fa-times"></i> Detener cámara
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group text-center my-2 pb-2">
                                    <button type="button" id="btn_recepcionar_pago_agenda"
                                        onclick="recepcion_pago();" class="btn btn-success">
                                        Recepcionar
                                    </button>
                                </div>
                            </div>
...
<script>
    var streamQrPagoAgenda = null;
    var intervaloQrPagoAgenda = null;

    function normalizarCodigoQrPagoAgenda(valor) {
        ...
        try {
            ...
            return String(datos.o || datos.orden_id || datos.numero_bono || datos.codigo || texto);
        } catch (errorJson) {
            try {
    ...
    function aplicarCodigoQrPagoAgenda(valor) {
        var codigo = normalizarCodigoQrPagoAgenda(valor);
        if (!codigo) {
            return;
        }

        $('#bono_numero').val(codigo).trigger('change');
        $('#estado_lector_qr_pago')
            .removeClass('text-danger text-success')
            .addClass('text-muted')
            .html('<i class="fas fa-spinner fa-spin"></i> Registrando QR en el flujo de caja...');

        $.ajax({
            url: '/Agenda/Hora-medica/hora/bono/qr/recibir-pendiente',
            type: 'POST',
            data: {
                _token: CSRF_TOKEN,
            ...
            if (!respuesta || Number(respuesta.estado) !== 1) {
                $('#estado_lector_qr_pago')
                    .removeClass('text-muted text-success')
                    .addClass('text-danger')
                    .text(respuesta && respuesta.msj ? respuesta.msj : 'No se pudo registrar el QR.');
                return;
            }

            var cobroHabilitado = Boolean(respuesta.cobro_habilitado);
            $('#modal_recepcion_bonos_api').data('qr-pendiente', !cobroHabilitado);
            $('#btn_recepcionar_pago_agenda')
                .prop('disabled', !cobroHabilitado)
                .toggleClass('btn-success', cobroHabilitado)
                .toggleClass('btn-secondary', !cobroHabilitado)
                .html(cobroHabilitado
                    ? '<i class="fas fa-check"></i> Recepcionar pago'
                    : '<i class="fas fa-clock"></i> QR recibido · Pendiente de atención');

            $('#estado_lector_qr_pago')
                .removeClass('text-muted text-danger')
                .addClass(cobroHabilitado ? 'text-success' : 'text-warning')
                .html('<i class="fas fa-check-circle"></i> ' + $('<div>').text(respuesta.msj).html());

            // Reflejar de inmediato en FullCalendar el estado "Espera" (morado).
            if (!cobroHabilitado && window.CalendarEl) {
                var eventoQr = window.CalendarEl.getEventById(String($('#bono_hora_medica').val()));
                if (eventoQr) {
                    eventoQr.setProp('backgroundColor', respuesta.color_hora || '#A06CC1');
                    eventoQr.setProp('borderColor', respuesta.color_hora || '#A06CC1');
                }
                window.CalendarEl.refetchEvents();
            } else if (!cobroHabilitado && typeof CalendarEl !== 'undefined' && CalendarEl) {
                var eventoQrLocal = CalendarEl.getEventById(String($('#bono_hora_medica').val()));
                if (eventoQrLocal) {
                    eventoQrLocal.setProp('backgroundColor', respuesta.color_hora || '#A06CC1');
                    eventoQrLocal.setProp('borderColor', respuesta.color_hora || '#A06CC1');
                }
                CalendarEl.refetchEvents();
            }

            detenerLectorQrPagoAgenda(false);
        }).fail(function (xhr) {
            var mensaje = xhr.responseJSON && xhr.responseJSON.message
                ? xhr.responseJSON.message
                : 'No se pudo registrar el QR en el flujo de caja.';
            $('#estado_lector_qr_pago')
                .removeClass('text-muted text-success')
                .addClass('text-danger')
                .text(mensaje);
    ...
    async function iniciarLectorQrPagoAgenda() {
        var estado = document.getElementById('estado_lector_qr_pago');

        if (!window.isSecureContext || !navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            $('#panel_lector_qr_pago').show();
            $(estado).removeClass('text-muted text-success').addClass('text-danger')
                .text('La cámara no está disponible. Puede ingresar el código manualmente.');
            $('#bono_numero').focus();
            return;
        }
        ...
        if (!('BarcodeDetector' in window)) {
            $('#panel_lector_qr_pago').show();
            $(estado).removeClass('text-muted text-success').addClass('text-danger')
                .text('Este navegador no admite lectura QR. Puede ingresar el código manualmente.');
            $('#bono_numero').focus();
            return;
        }
        ...
        detenerLectorQrPagoAgenda(false);
        $('#panel_lector_qr_pago').show();
        $(estado).removeClass('text-danger text-success').addClass('text-muted')
            .text('Apunte la cámara al QR emitido en Bonos Veterinaria.');

        try {
            var detector = new BarcodeDetector({ formats: ['qr_code'] });
            streamQrPagoAgenda = await navigator.mediaDevices.getUserMedia({
                video: { facingMode: { ideal: 'environment' } },
                audio: false
            });

            var video = document.getElementById('video_lector_qr_pago');
            video.srcObject = streamQrPagoAgenda;
            await video.play();

            intervaloQrPagoAgenda = window.setInterval(async function () {
                if (!video.videoWidth) {
                    return;
                }
                ...
                try {
                    ...
                    if (codigos.length) {
                        aplicarCodigoQrPagoAgenda(codigos[0].rawValue);
                    }
                } catch (errorLectura) {
                    // La cámara continúa intentando hasta detectar un QR válido.
                }
            }, 350);
        } catch (errorCamara) {
            detenerLectorQrPagoAgenda(false);
            $(estado).removeClass('text-muted text-success').addClass('text-danger')
                .text('No se pudo abrir la cámara. Autorice su uso o ingrese el código manualmente.');
            $('#bono_numero').focus();
        }
    }

    function detenerLectorQrPagoAgenda(ocultarPanel) {
        if (intervaloQrPagoAgenda) {
            window.clearInterval(intervaloQrPagoAgenda);
            intervaloQrPagoAgenda = null;
        }

        if (streamQrPagoAgenda) {
            streamQrPagoAgenda.getTracks().forEach(function (track) {
                track.stop();
            });
            streamQrPagoAgenda = null;
        }
        ...
        if (ocultarPanel !== false) {
            $('#panel_lector_qr_pago').hide();
        }
    }

    $(document).on('hidden.bs.modal', '#modal_recepcion_bonos_api', function () {
        detenerLectorQrPagoAgenda();
    });
</script>
